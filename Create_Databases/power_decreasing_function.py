...
def read_data(path, keywords, keywords_index, start_index, end_index, index_margin) -> object:
    ...
    for i_value in np.arange(start_index, end_index, index_margin):
        ...
        if os.path.exists(file):
            array_name = temp_array[0] + '_' + temp_array[1] + str(i_value)
            temp_data = []
...
def drop_table(path, keywords, keywords_index, start_index, end_index, margin) -> object:
    ...
    for i_value in np.arange(start_index, end_index, margin):
        i_value = int(i_value) if i_value == int(i_value) else i_value
        file = path + '\\' + keywords[keywords_index] + str(i_value) + '.txt'
        temp_array = np.array(keywords[keywords_index].split(' '))

        if os.path.exists(file):
            if i_value == int(i_value):
                table_name = temp_array[0] + '_' + temp_array[1] + str(i_value)
            else:
                temp_string_1 = str(i_value).split('.')
                table_name = temp_array[0] + '_' + temp_array[1] + temp_string_1[0] + '_' + temp_string_1[1]
    ...
    for i_value in np.arange(start_index, end_index, margin):
        ...
        if os.path.exists(file):
            if i_value == int(i_value):
                ...
                temp_string = str(i_value).split('.')
                table_name = temp_array[0] + '_' + temp_array[1] + temp_string[0] + '_' + temp_string[1]
