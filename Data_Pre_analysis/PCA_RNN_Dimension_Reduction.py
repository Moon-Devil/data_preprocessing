...
for i_value in range(low_dimension, high_dimension):    # low_dimension, high_dimension
    ...
    PCA_RNN_history = model.fit(x_train, y_train, batch_size=batch_size, verbose=0, epochs=epochs_size,
    ...
    PCA_RNN_accuracy[i_value] = accuracy

    if PCA_RNN_accuracy[i_value] < best_accuracy:
        best_accuracy = PCA_RNN_accuracy[i_value]
        PCA_RNN_loss = PCA_RNN_history.history['loss']
        PCA_RNN_MSE = PCA_RNN_history.history['mean_squared_error']
        PCA_RNN_valid_loss = PCA_RNN_history.history['val_loss']
        PCA_RNN_valid_MSE = PCA_RNN_history.history['val_mean_squared_error']
        best_dimension = i_value

PCA_RNN_accuracy_length = len(PCA_RNN_accuracy)
...
file = path + "\\PCA_Dimension_Reduction_PCA_RNN.txt"
if os.path.exists(file):
    os.remove(file)

with open(file, "w+") as f:
    f.write(str("====================================PCA_RNN=================================\n"))
    for i_value in np.arange(PCA_RNN_accuracy_length):
        index = i_value + low_dimension
        f.write("Dimension=" + str(index) + "\t" + "accuracy=" + str(PCA_RNN_accuracy[index]) + "\n")

    f.write("Best dimension=" + str(best_dimension) + "\n")
    f.write("PCA_RNN_loss_length= " + str(len(PCA_RNN_loss)) + "\n")
    f.write("PCA_RNN_MSE_length= " + str(len(PCA_RNN_MSE)) + "\n")
    f.write("PCA_RNN_val_loss_length= " + str(len(PCA_RNN_valid_loss)) + "\n")
    f.write("PCA_RNN_val_MSE_length= " + str(len(PCA_RNN_valid_MSE)) + "\n")

    f.write("PCA_NN_loss=" + "\t")
    for i_value in np.arange(len(PCA_RNN_loss)):
        if i_value != (len(PCA_RNN_loss) - 1):
            f.write(str(PCA_RNN_loss[i_value]) + ",")
        else:
            f.write(str(PCA_RNN_loss[i_value]) + "\n")

    f.write("PCA_NN_MSE=" + "\t")
    for i_value in np.arange(len(PCA_RNN_MSE)):
        if i_value != (len(PCA_RNN_MSE) - 1):
            f.write(str(PCA_RNN_MSE[i_value]) + ",")
        else:
            f.write(str(PCA_RNN_MSE[i_value]) + "\n")

    f.write("PCA_NN_val_loss=" + "\t")
    for i_value in np.arange(len(PCA_RNN_valid_loss)):
        if i_value != (len(PCA_RNN_valid_loss) - 1):
            f.write(str(PCA_RNN_valid_loss[i_value]) + ",")
        else:
            f.write(str(PCA_RNN_valid_loss[i_value]) + "\n")

    f.write("PCA_NN_val_MSE=" + "\t")
    for i_value in np.arange(len(PCA_RNN_valid_MSE)):
        if i_value != (len(PCA_RNN_valid_MSE) - 1):
            f.write(str(PCA_RNN_valid_MSE[i_value]) + ",")
        else:
            f.write(str(PCA_RNN_valid_MSE[i_value]) + "\n")
