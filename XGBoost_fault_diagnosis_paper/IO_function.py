...
def write_to_text(filename, data_lists, flag):
    ...
    result_directory = father_path + 'XGBoost_paper\\'
...
def recode_history(history):
    data_lists = [history.best_params_['learning_rate']]
    write_to_text('learning_rate', data_lists, "w+")

    data_lists = [history.best_params_['n_estimatores']]
    write_to_text('n_estimatores', data_lists, "w+")

    data_lists = [history.best_score_]
    write_to_text('best_score', data_lists, "w+")

    data_lists = history.cv_results_['mean_fit_time'].tolist()
    write_to_text('mean_fit_time', data_lists, "w+")

    data_lists = history.cv_results_['std_fit_time'].tolist()
    write_to_text('std_fit_time', data_lists, "w+")

    data_lists = history.cv_results_['mean_score_time'].tolist()
    write_to_text('mean_score_time', data_lists, "w+")

    data_lists = history.cv_results_['std_score_time'].tolist()
    write_to_text('std_score_time', data_lists, "w+")

    data_lists = history.cv_results_['param_learning_rate'].tolist()
    write_to_text('param_learning_rate', data_lists, "w+")

    data_lists = history.cv_results_['param_n_estimatores'].tolist()
    write_to_text('parameter_n_estimatores', data_lists, "w+")

    data_lists = history.cv_results_['mean_test_score'].tolist()
    write_to_text('mean_test_score', data_lists, "w+")

    data_lists = history.cv_results_['std_test_score'].tolist()
    write_to_text('std_test_score', data_lists, "w+")

    data_lists = history.cv_results_['mean_train_score'].tolist()
    write_to_text('mean_train_score', data_lists, "w+")

    data_lists = history.cv_results_['std_train_score'].tolist()
    write_to_text('std_train_score', data_lists, "w+")
