...
batch_size = 32     # AE批处理长度
epochs_size = 2000     # AE循环代数

# AE-高斯过程回归降维
AE_GPR_history = dict()
AE_GPR_accuracy = dict()
AE_GPR_y_predict = dict()
AE_GPR_y_test = dict()
history = dict()
...
file = path + "\\AE_Dimension_Reduction_AE_GPR.txt"
...
with open(file, "w+") as f:
    ...
    for i_value in np.arange(AE_GPR_accuracy_i_value):
        ...
        temp_mean = np.mean(AE_GPR_accuracy[index]) * (-1)
